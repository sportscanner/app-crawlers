...
class UserRepository(FirebaseManager):
    def create_user(self, user_data : UserInCreate) -> UserOutput:
        ...
        user_data_dict["created_at"] = datetime.utcnow().isoformat()  # Add a UTC timestamp
        doc_ref.set(user_data_dict)
        logging.info(f"Document added to collection '{settings.FIRESTORE_USER_COLLECTION}' with ID: {doc_ref.id}")
        return UserOutput(
            id = doc_ref.id,
    ...
    def get_user_by_email(self, email: str) -> Optional[User]:
        user_docs = (
            ...
            .where(field_path='email', op_string='==', value=email).get()
        )
        if len(user_docs) > 0:
...
if __name__ == "__main__":
    ...
    print(
        userService.update_user(
            ...
            updates = {
                "preferredVenues": ["Layman centre", "Talacre"]
            }
        )
    )
