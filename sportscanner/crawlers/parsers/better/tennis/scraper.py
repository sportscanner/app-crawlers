...
class BetterLeisureTennisRequestStrategy(AbstractRequestStrategy):
    """
    If there are multiple variations like indoor/outdoor courts, add those here
    ...
    def generate_request_details(
        ...
        for activityId, fallback_activityId in activity_slug_pairs("tennis", sports_venue.slug):
...
class BetterLeisureTennisCrawler(BetterStyleCrawler):
    def __init__(self):
        super().__init__(
            request_strategy = BetterLeisureTennisRequestStrategy(),
...
def run(
    ...
    sport_venues_to_crawl: List[
        SportsVenue] = crawler.get_venues_by_sport_offering(sport="tennis")
...
def coroutines(search_dates: List[date]):
    return BetterLeisureTennisCrawler().coroutines(search_dates, sport="tennis", delta=6)
...
if __name__ == "__main__":
    ...
    _sport_venues_composite_ids = []

    logging.info("Running BetterLeisureTennisCrawler crawler")
    parsedResults = run(
        crawler = BetterLeisureTennisCrawler(),
        search_dates = _dates,
        sport_venues_composite_ids = _sport_venues_composite_ids
    )
    logging.success(f"BetterLeisureTennisCrawler finished. Got {len(parsedResults)} results.")
