...
from sportscanner.crawlers.parsers.haringey.core.schema import HaringeyApiResponseSchema
from sportscanner.crawlers.parsers.core.schemas import (UnifiedParserSchema)
from sportscanner.crawlers.parsers.utils import validate_api_response


class HaringeyLeisureResponseParserStrategy(AbstractResponseParserStrategy):
    def _transform_raw_response_to_typed(self, api_response) -> List[HaringeyApiResponseSchema]:
        aligned_api_response = []
        if isinstance(api_response, dict):
            try:
                aligned_api_response.extend(
                    [
                        HaringeyApiResponseSchema(**response_block)
                        for _key, response_block in api_response.items()
                    ]
                )
            except ValidationError as e:
                logging.error(
                    f"Unable to apply haringey API response schema to raw API json:\n{e}"
                )
            ...
            if len(api_response) > 0:
                try:
                    ...
                    logging.error(
                        f"Unable to apply HaringeyApiResponseSchema to raw API json:\n{e}"
                    )
        logging.debug(f"Data aligned with overall schema: {HaringeyApiResponseSchema}")
        return aligned_api_response

    @override
    def parse(self, raw_response: RawResponseData) -> List[UnifiedParserSchema]:
        raw_response_typed: List[HaringeyApiResponseSchema] = self._transform_raw_response_to_typed(raw_response.content)
...
class HaringeyLeisureTaskCreationStrategy(AbstractAsyncTaskCreationStrategy):
