...
def generate_api_call_params(
    ...
    headers = {
        ...
        "referer": f"https://lambethcouncil.bookings.flow.onl/location/{sports_centre.slug}/{activity}/{fetch_date}/by-time",
...
async def fetch_data(
    ...
    if validated_response_data is not None:
        ...
        return [
            UnifiedParserSchema.from_active_lambeth_api_response(response, metadata)
...
def pipeline(
    ...
    logging.warning(
        f"Search dates for Active Lambeth crawler narrowed down to: {formatted_date_list(allowable_search_dates)}"
    )
    sports_centre_lists: List[db.SportsVenue] = db.get_all_rows(
        ...
        .where(db.SportsVenue.organisation_website == "https://active.lambeth.gov.uk/"),
    )
    logging.info(
        f"{len(sports_centre_lists)} Active Lambeth Sports venues queried from database"
    )
