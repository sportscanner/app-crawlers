...
def generate_url() -> str:
    ...
    return "https://towerhamletscouncil.gladstonego.cloud/api/availability/V2/sessions?siteIds=MEPLS&activityIDs=MACT000010&webBookableOnly=true&dateFrom=2025-01-29T00:00:00.000Z&locationId="


response = httpx.get(generate_url(), headers=generate_headers())
content_type = response.headers.get("content-type", "")
match response.status_code:
    case 200:
        json_response = response.json()
        _modelling: List[TowerHamletsResponseSchema] = [
            TowerHamletsResponseSchema(**x) for x in json_response
        ]
    ...
    case _:
        logging.error(
            f"Response status code is not: Response [200 OK]" f"\nResponse: {response}"
        )
