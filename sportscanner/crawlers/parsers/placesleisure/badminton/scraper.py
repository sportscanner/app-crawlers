...
class _StubRequestStrategy(AbstractRequestStrategy):
    """Places Leisure bypasses the standard per-venue request loop entirely
    (see core/strategy.py's module docstring for why - a two-phase
    schedule-then-availability fetch doesn't fit the single-request-per-venue-
    per-date shape). Only exists to satisfy BaseCrawler's constructor."""
...
class PlacesLeisureBadmintonCrawler(BaseCrawler):
    """See sportscanner/crawlers/parsers/placesleisure/core/strategy.py for
    the full two-phase fetch design (schedule scrape + per-slot availability
    calls). Same bypass-ScraperCoroutines pattern as Matchi/Playtomic/CitySport."""
    ...
    def __init__(self):
        ...
        self._fetcher = PlacesLeisureSlotFetcher(activity_group="BADMINTON", category="Badminton")
    ...
    async def _crawl_async(
        ...
        logging.info(f"Places Leisure: crawling {len(matched)} venue(s) for badminton")
...
def coroutines(search_dates: List[date]) -> Coroutine[Any, Any, List[UnifiedParserSchema]]:
    ...
    crawler = PlacesLeisureBadmintonCrawler()
    venues = crawler.get_venues_by_sport_offering(sport="badminton")
    if not venues:
        logging.warning("Places Leisure: no badminton venues found in DB - skipping")
...
if __name__ == "__main__":
    ...
    print(f"Places Leisure badminton test run for dates: {_dates}")
    crawler = PlacesLeisureBadmintonCrawler()
    venues = crawler.get_venues_by_sport_offering(sport="badminton")
    if not venues:
        print("[yellow]No Places Leisure badminton venues in DB. Add entries to venues.json first.[/yellow]")
    else:
