...
from sportscanner.crawlers.parsers.uelsportsdock.core.schema import (
    UELSportsDockResponseSchema,
)
...
class UELSportsDockResponseParserStrategy(AbstractResponseParserStrategy):
    def _transform_raw_response_to_typed(
        self, api_response
    ) -> List[UELSportsDockResponseSchema]:
        try:
            ...
            logging.debug(
                f"Data aligned with overall schema: {UELSportsDockResponseSchema}"
            )
            return aligned_api_response
        except ValidationError as e:
            logging.error(
                f"Unable to apply UELSportsDockResponseSchema to raw API json:\n{e}"
            )
    ...
    def parse(self, raw_response: RawResponseData) -> List[UnifiedParserSchema]:
        if len(raw_response.content) <= 0:
            return []
        raw_response_typed: List[UELSportsDockResponseSchema] = (
