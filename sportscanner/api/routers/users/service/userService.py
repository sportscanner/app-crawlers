...
from sportscanner.api.routers.users.schema.user import (UserOutput, UserInCreate, UserInLogin, UserWithToken)
...
class UserService:
    ...
    def get_user_info(self, user_id: str) -> dict:
        try:
            if not self.__userRepository.user_exists_by_id(user_id=user_id):
                raise HTTPException(status_code=400, detail=f"No user details found for id: {user_id}")
            ...
            print(error)
            raise HTTPException(status_code=500, detail="Failed to retrive information from Users repo") from error
    ...
    def signup(self, user_details: UserInCreate) -> UserWithToken:
        try:
            ...
            new_user: UserOutput = self.__userRepository.create_user(user_data=user_details)
    ...
    def login(self, login_details: UserInLogin) -> UserWithToken:
        try:
            if not self.__userRepository.user_exists_by_email(email=login_details.email):
                raise HTTPException(status_code=400, detail="No account found. Please create an account.")
...
if __name__ == "__main__":
    ...
    print(
        _UserService.get_user_info(user_id="sjsjsj")
    )