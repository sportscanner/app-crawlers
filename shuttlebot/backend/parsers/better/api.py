...
def generate_api_call_params(sports_centre: SportsCentre, fetch_date: date, activity: str):
...
async def fetch_data(client, url, headers):
    ...
    if validated_response_data is not None:
        ...
        return [UnifiedParserSchema.from_better_api_response(response) for response in
    ...
    if isinstance(api_response, dict):
        try:
            aligned_api_response.extend(
                [BetterApiResponseSchema(**response_block) for _key, response_block in
                                         api_response.items()]
            )
        ...
        if len(api_response) > 0:
            try:
                aligned_api_response.extend(
                    [BetterApiResponseSchema(**response_block) for response_block in api_response]
                )
            except ValidationError as e:
                logging.error(f"Unable to apply Better API response schema to raw API json:\n{e}")
